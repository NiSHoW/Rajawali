...
public class Camera extends ATransformable3D {
	...
	public boolean showBoundingVolume() {
		return mShowBoundingVolume;
	}
	
	public void showBoundingVolume(boolean show) {
		mShowBoundingVolume = show;
	}
	...
	public void render(Camera camera, final Matrix4 vpMatrix, final Matrix4 projMatrix, final Matrix4 vMatrix, 
			...
			if(mBox == null){
				double size = camera.getPosition().distanceTo(getPosition()) / 100;
				mBox = new Cube((float) size);
				mBox.isBoundingVolume(true);
				Material material = new Material();
				mBox.setMaterial(material);
				mBox.setColor(Color.RED);
				mBox.setDrawingMode(GLES20.GL_LINE_LOOP);
				mBox.setDoubleSided(true);
			}
			
			mBox.setPosition(getPosition());
			mBox.setOrientation(this.mOrientation);
			mBox.render(camera, vpMatrix, projMatrix, vMatrix, parentMatrix, pickerInfo);	
